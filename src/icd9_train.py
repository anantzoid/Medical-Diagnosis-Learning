...
def basicstats(patients):
    ...
    print "FD:", np.histogram(num_seq, bins=[0, 2, 5,10,20,50,100,500,1000,10000]) 
...
def padding_collation(batch):
    ...
    for datum in batch:
        padded_vec = datum[0] + [0 for i in range(max_seq_len-len(datum[0]))]
...
class Trainer():
    def __init__(self, train_loader, embeddings, token2idx, idx2token, test_loader, batch_size):
        ...
        self.hidden_dim = 100
        self.lr = 1e-3
        self.model = LSTMModel(len(self.token2idx.keys()), self.embedding_size, self.hidden_dim, self.batch_size)
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            self.lr *= 0.9
            self.opti = torch.optim.Adam(self.model.parameters(), lr=self.lr, betas=(0.5, 0.999))
        self.plot(step_log, loss_lof, val_acc_log, val_loss_log)
...
if __name__ == "__main__":
    ...
    if os.path.exists(datapath):
        ...
        patients, pat_adm_id, token2idx = build_encounter_seq(os.path.join(base_path, 'mimic3/DIAGNOSES_ICD.csv'))
        #POC_of_seq(patients, pat_adm_id)
        basicstats(patients)
        db = ICD9DataBuilder(patients, labels, embeddings, token2idx)
        db.label_data()
        f = open(datapath, 'w')
    ...
    trainer = Trainer(train_loader, embeddings, token2idx, idx2token, test_loader, batch_size) 
    trainer.train()
