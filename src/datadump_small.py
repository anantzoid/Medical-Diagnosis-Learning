...
def build_notes_dump(notes_path, data):
    with open(notes_path, 'rb') as csvf:
        ...
        for row in csvreader:
            ...
            if num % 10000 == 0:
              print("Processed {} notes".format(num))
            if num % 100000 == 0:
              break
    return data
...
def make_data_dump():
    ...
    with open('/misc/vlgscratch2/LecunGroup/laura/medical_notes/notes_dump_small.pkl', 'w') as f:
...
'''
def get_data_stats():
    with open('/misc/vlgscratch2/LecunGroup/laura/medical_notes/notes_dump_small.pkl') as f:
        data = pickle.load(f)
