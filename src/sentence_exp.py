exp_name = 'summaries_50label_vocab50_maxseq500_paddingrear'
...
for nu_ep in range(num_epochs):
    ...
    if nu_ep%5==0:
        ...
        lr *= 0.9
        opti = torch.optim.Adam(model.parameters(), lr=lr, betas=(0.5, 0.999))
        torch.save(model.state_dict(), os.path.join(base_path, 'model_%s.pth'%exp_name))


    f = open(os.path.join(base_path, 'results_%s.pkl'%exp_name), 'w')
    ...
    #plt.savefig(os.path.join(base_path, 'plot_%s.png'%exp_name))
