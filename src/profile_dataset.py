...
def count_unks(data):
    ...
    for elem in data:
        for sent in elem[1]:
            ...
            toks += len(sent)
    print("Toks: {} Unks: {} % UNKs{:.3f}", toks, unks, unks / float(toks))
...
def get_stats(data):
    ...
    print("Avg sents: {:.3f}, Min sents: {}: Max sents: {}",
          np.mean(sents_per_note), max(sents_per_note), min(sents_per_note))
    print("Avg words: {:.3f}, Min words: {}: Max words: {}",
          np.mean(words_per_sent), max(words_per_sent), min(words_per_sent))
